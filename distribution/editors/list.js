;(function(Form) {
  ...
  Form.editors.List = Form.editors.Base.extend({
    ...
    validate: function() {
      ...
      if (validators) {
        ...
        if (error) {
          list_errors.push(error);
        }
      }

      //Check if any item has errors
      var hasErrors = _.compact(errors).length || _.compact(list_errors).length ? true : false;
  ...
  Form.editors.List.Item = Form.editors.Base.extend({
    ...
    validate: function() {
      ...
      if (!validators && !this.editor.validate) return null;

      var error = null;
      //Run through editor validation
